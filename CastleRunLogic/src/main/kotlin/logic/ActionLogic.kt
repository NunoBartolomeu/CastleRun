...
class ActionLogic {
    companion object {
        fun deploy(game: Game, player: Player, entry: Position, to: Position, distance: Int) {
            GameValidator.isValidDeploy(game, player, entry, to, distance)
    ...
    fun useItem(game: Game, player: Player, position: Position) {
        TODO()
    }

    fun duel(game: Game, player: Player, position: Position, enemyPlayer: Player, enemyPosition: Position) {
        TODO()
    }
}